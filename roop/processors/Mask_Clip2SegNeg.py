...
class Mask_Clip2SegNeg():

    model_clip = None

    processorname = 'clip2seg_neg'
    type = 'mask'
    ...
    def Run(self, img1, keywords:str) -> Frame:
        ...
        img_mask = cv2.rectangle(
            img_mask, (mask_border+int(l), mask_border+int(t)), 
        ...
        clip_mask = 1.0 - torch.sigmoid(preds[0][0])
        
        for i in range(len(prompts)-1):
            clip_mask += 1.0 - torch.sigmoid(preds[i+1][0])
