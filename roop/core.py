...
def pre_check() -> bool:
    ...
    util.conditional_download(
        download_directory_path,
        ["https://huggingface.co/dnnagy/RestoreFormer/resolve/main/RestoreFormer.onnx"],
    )

    download_directory_path = util.resolve_relative_path(
        "../models/RestoreFormerPlusPlus"
    )
    util.conditional_download(
        download_directory_path,
        [
...
def get_processing_plugins(use_clip):
    processors = "faceswap"
    if use_clip:
        processors += ",mask_clip2seg_pos"
        processors += ",mask_clip2seg_neg"
...
def preview_mask_pos(frame, clip_text):
    import numpy as np
    ...
    maskprocessor = next(
        (x for x in process_mgr.processors if x.processorname == "clip2seg"), None
    )

    return process_mgr.process_mask(maskprocessor, frame, maskimage)


def preview_mask_neg(frame, clip_text):
    import numpy as np
...
def batch_process(
    files: list[ProcessEntry], use_clip, clip_text_pos, clip_text_neg, use_new_method, progress
) -> None:
