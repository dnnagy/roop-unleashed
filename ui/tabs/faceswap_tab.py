...
def faceswap_tab():
        ...
        with gr.Row(variant="panel"):
            ...
            with gr.Column(scale=2):
                previewimage = gr.Image(
                    label="Preview Image", height=576, interactive=False
                )
                with gr.Row(variant="panel"):
                ...
                with gr.Row():
                    preview_frame_num = gr.Slider(
                        0, 0, value=0, label="Frame Number", step=1.0, interactive=True
                    )
                with gr.Row():
        ...
        with gr.Row(variant="panel"):
            with gr.Column(scale=1):
                selected_face_detection = gr.Dropdown(
                    ...
                    value="First found",
                    label="Select face selection for swapping",
                )
                max_face_distance = gr.Slider(
                    0.01, 1.0, value=0.65, label="Max Face Similarity Threshold"
                )
            ...
            with gr.Column(scale=1):
                ui.globals.ui_selected_enhancer = gr.Dropdown(
                    ["None", "Codeformer", "RestoreFormer", "RestoreFormerPlusPlus", "DMDNet", "GFPGAN", "GPEN"],
                    value="None",
    ...
    input_faces.select(on_select_input_face, None, None).then(
        ...
        outputs=[previewimage, mask_top, mask_bottom],
    )
...
def on_srcfile_changed(srcfiles, progress=gr.Progress()):
    ...
    for f in srcfiles:
        ...
        if source_path.lower().endswith("fsz"):
            ...
            progress(
                0,
                desc="Retrieving faces from image",
            )
...
def on_use_face_from_selected(files, frame_num):
    ...
    if len(thumbs) == 1:
        ...
        return (
            ...
            gr.Dropdown.update(value="Selected face"),
        )
...
def on_destfiles_changed(destfiles):
    ...
    if total_frames > 0:
        return gr.Slider.update(value=0, maximum=total_frames), gen_processing_text(
            list_files_process[idx].startframe, list_files_process[idx].endframe
        )
...
def on_destfiles_selected(evt: gr.SelectData):
    ...
    if total_frames > 0:
        return (
            gr.Slider.update(
                value=list_files_process[idx].startframe, maximum=total_frames
            ),
            gen_processing_text(
                list_files_process[idx].startframe, list_files_process[idx].endframe
            ),
    ...
    return (
        gr.Slider.update(value=0, maximum=total_frames),
